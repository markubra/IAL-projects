...
void Stack_Init( Stack *stack ) {
    // If stack is NULL -- calls an error
    if (stack == NULL)
        Stack_Error(SERR_INIT);
    // If not then set top of the stack to -1
    else
        stack->topIndex = -1;
}
...
int Stack_IsEmpty( const Stack *stack ) {
    // If stack is not empty returns 0 else 1
...
int Stack_IsFull( const Stack *stack ) {
    // If topIndex of stack is less then size of the stack - 1
    // (-1 because indexing starts from 0) returns 0 else 1
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    // If stack is empty call an error else set the pointer to the first element in stack
...
void Stack_Pop( Stack *stack ) {
    // If stack is not empty decrease topIndex by 1
    if (!Stack_IsEmpty(stack))
        stack->topIndex--;
}
...
void Stack_Push( Stack *stack, char data ) {
    // If stack is full call an error
    if (Stack_IsFull(stack))
        Stack_Error(SERR_PUSH);
    // Else increase topIndex and add a new data on the top
    else
    {
        stack->topIndex++;
